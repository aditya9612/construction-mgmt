...
async def check_in(
    ...
    if project_id:
        ...
        if project:
            ...
            if work_location_type != "WFH" and project.latitude and project.longitude and check_in_latitude and check_in_longitude:
            ...
            if project.shift_start_time:
                shift_start_dt = datetime.combine(attendance_date, project.shift_start_time)
                ...
                if actual_in_time > shift_start_with_grace:
                    is_late = True
                    late_minutes = int((actual_in_time - shift_start_dt).total_seconds() / 60)
...
async def check_out(
    ...
    result = await db.execute(
        select(UserAttendance)
        .where(UserAttendance.id == attendance_id, UserAttendance.user_id == current_user.id)
    )
    ...
    if attendance.project_id:
        project_result = await db.execute(select(Project).where(Project.id == attendance.project_id))
        project = project_result.scalars().first()
        if project and project.shift_end_time:
            shift_end_dt = datetime.combine(attendance.attendance_date, project.shift_end_time)
            if actual_out_time < shift_end_dt:
                is_early_departure = True
                early_minutes = int((shift_end_dt - actual_out_time).total_seconds() / 60)
    ...
    # Update remarks if provided at checkout
    if remarks:
...
async def list_attendance(
    ...
    stmt = stmt.order_by(desc(UserAttendance.attendance_date)).offset(offset).limit(page_size)
