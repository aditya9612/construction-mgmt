...
def upgrade():
    ...
    op.create_index(op.f('ix_quotation_master_quotation_no'), 'quotation_master', ['quotation_no'], unique=True)
    ...
    sa.Column('remaining_quantity', sa.DECIMAL(precision=18, scale=2), nullable=True),
    sa.Column('completion_percentage', sa.DECIMAL(precision=5, scale=2), nullable=True),
    sa.Column('status', sa.Enum('ON_TRACK', 'DELAY', 'COMPLETED', 'NOT_STARTED', name='workactivitystatus'), nullable=False),
    ...
    op.create_table('daily_progress_entries',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.Column('created_at', sa.TIMESTAMP(), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['activity_id'], ['work_activities.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.ForeignKeyConstraint(['quotation_id'], ['quotation_master.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('quotation_labour',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('quotation_id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_quotation_labour_quotation_id'), 'quotation_labour', ['quotation_id'], unique=False)
    op.create_index(op.f('ix_quotation_labour_skill_type'), 'quotation_labour', ['skill_type'], unique=False)
    op.create_table('quotation_extra_charges',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('quotation_id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_quotation_extra_charges_expense_type'), 'quotation_extra_charges', ['expense_type'], unique=False)
    op.create_table('quotation_materials',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('quotation_id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_invoices_quotation_id'), 'invoices', ['quotation_id'], unique=True)
    op.create_foreign_key(None, 'invoices', 'quotation_master', ['quotation_id'], ['id'], ondelete='SET NULL')
    op.add_column('ra_bills', sa.Column('quotation_id', sa.Integer(), nullable=True))
    op.create_index(op.f('ix_ra_bills_quotation_id'), 'ra_bills', ['quotation_id'], unique=True)
    op.create_foreign_key(None, 'ra_bills', 'quotation_master', ['quotation_id'], ['id'], ondelete='SET NULL')
    op.alter_column('safety_incidents', 'safety_checklist_status',
    ...
    op.alter_column('safety_incidents', 'ppe_compliance',
               existing_type=mysql.TINYINT(display_width=1),
               nullable=True,
               existing_server_default=sa.text("'1'"))
    op.add_column('suppliers', sa.Column('supplier_name', sa.String(length=255), nullable=False))
    op.add_column('suppliers', sa.Column('contact_person', sa.String(length=255), nullable=True))
    op.add_column('suppliers', sa.Column('phone_email', sa.String(length=100), nullable=True))
    op.add_column('suppliers', sa.Column('gst_number', sa.String(length=20), nullable=True))
    ...
    op.alter_column('tasks', 'created_by_user_id',
               existing_type=mysql.INTEGER(),
               nullable=False)
    op.drop_index(op.f('ix_tasks_project_id'), table_name='tasks')
    op.drop_index(op.f('ix_tasks_title'), table_name='tasks')
    op.add_column('work_orders', sa.Column('quotation_id', sa.Integer(), nullable=True))
    op.create_index(op.f('ix_work_orders_quotation_id'), 'work_orders', ['quotation_id'], unique=True)
    op.create_foreign_key(None, 'work_orders', 'quotation_master', ['quotation_id'], ['id'], ondelete='SET NULL')
...
def downgrade():
    ...
    op.alter_column('safety_incidents', 'ppe_compliance',
               ...
               existing_server_default=sa.text("'1'"))
    op.alter_column('safety_incidents', 'safety_checklist_status',
               existing_type=sa.Enum('PENDING', 'COMPLETED', 'FAILED', name='safetycheckliststatus'),
