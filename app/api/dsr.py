...
async def create_dsr(
    ...
    if payload.report_date > date.today():
        raise HTTPException(status_code=400, detail="Future report date not allowed")

    if payload.labour_count < 0:
        raise HTTPException(status_code=400, detail="Labour count cannot be negative")

    if not payload.work_done.strip():
        raise HTTPException(status_code=400, detail="Work done cannot be empty")

    if payload.weather not in ["Sunny", "Rainy", "Cloudy", "Windy"]:
        raise HTTPException(status_code=400, detail="Invalid weather value")
...
async def get_project_dsr(
    ...
    result = await db.execute(
        select(DailySiteReport).where(DailySiteReport.project_id == project_id)
    )
...
async def update_dsr(
    ...
    if payload.report_date is not None and payload.report_date > date.today():
        raise HTTPException(status_code=400, detail="Future report date not allowed")

    if payload.labour_count is not None and payload.labour_count < 0:
        raise HTTPException(status_code=400, detail="Labour count cannot be negative")

    if payload.work_done is not None and not payload.work_done.strip():
        raise HTTPException(status_code=400, detail="Work done cannot be empty")

    if payload.weather is not None and payload.weather not in [
        "Sunny",
        "Rainy",
        "Cloudy",
        "Windy",
