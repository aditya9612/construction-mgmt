...
async def create_user(
    ...
    if payload.email and not payload.password:
        raise AppError(status_code=422, message="Password required when creating with email")
    try:
    ...
    if payload.email:
        ...
        mobile_val = _normalize_mobile(payload.mobile_number) if payload.mobile_number else None
        mobile_val = mobile_val if (mobile_val and len(mobile_val) >= 10) else None
        if mobile_val:
            mob_exists = await db.scalar(select(User).where(User.mobile == mobile_val))
            if mob_exists:
...
async def list_users(
    ...
    if search:
        like = f"%{search}%"
        cond = or_(User.email.ilike(like), User.full_name.ilike(like), User.mobile.ilike(like))
...
async def update_user(
    ...
    if "mobile_number" in data:
        ...
        data["mobile"] = mobile_val if (mobile_val and len(mobile_val) >= 10) else None
        if data["mobile"]:
            existing = await db.scalar(select(User).where(and_(User.mobile == data["mobile"], User.id != user_id)))
            if existing:
                raise ConflictError("Mobile already registered")
    if "role" in data:
        try:
