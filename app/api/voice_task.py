...
async def process_voice_audio(
    ...
    language_code: VoiceLanguage = Form(
        VoiceLanguage.AUTO
    ),
    ...
    try:
        ...
        english_text = (
            VoiceService.translate_to_english(
                original_text,
                lang_code
            )
        )
        ...
        gen_filepath = (
            VoiceService.generate_task_audio(
                original_text,
                lang_code,
                GEN_DIR
            )
        ...
        return {
            ...
            "voice_instruction_url":
                f"/{raw_filepath}".replace(
                    "\\",
                    "/"
                ),

            "generated_audio_url":
                f"/{gen_filepath}".replace(
                    "\\",
                    "/"
                ),
...
async def assign_voice_task(
    ...
    """
    Create task using voice workflow.
    """
    ...
    return {
        "message":
            "Voice task assigned successfully",
