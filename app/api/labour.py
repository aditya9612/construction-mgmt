...
async def create_labour(
    ...
    # Extract user attributes to avoid MissingGreenlet after rollback
    ...
    for _ in range(3):
        try:
            ...
            obj = Labour(**data)
            obj.company_id = current_user_company_id
            db.add(obj)
            await db.flush()
            ...
            if "email" in error_str or "users.ix_users_email" in error_str:
                raise HTTPException(
                    status_code=400, detail="User with this email already exists"
                )
...
async def list_labour(
    ...
    query = (
        ...
        .where(Labour.company_id == current_user.company_id)
        .distinct()
    )
...
async def weekly_report(
    ...
    result = await db.execute(
        ...
        .order_by(extract("week", UserAttendance.attendance_date))
    )
    ...
    return [
        s.WeeklyReportOut(
            ...
            total_hours=Decimal(r.hours or 0),
            overtime_hours=Decimal(r.ot or 0),
            total_wage=(
...
@router.get(
    "/{labour_id}/monthly-report",
    response_model=list[s.MonthlyReportOut],
)
async def monthly_report(
    ...
    result = await db.execute(
        select(
            ...
            func.coalesce(
                func.sum(UserAttendance.working_hours),
                Decimal("0"),
            ).label("hours"),
            func.coalesce(
            ...
            func.coalesce(
                func.sum(UserAttendance.overtime_hours * UserAttendance.overtime_rate),
                Decimal("0"),
            ).label("ot_wage"),
        )
        .join(Labour, Labour.user_id == UserAttendance.user_id)
        ...
        .order_by(extract("month", UserAttendance.attendance_date))
    )
    ...
    for r in rows:
        total_hours = Decimal(r.hours or 0)
        overtime_hours = Decimal(r.ot or 0)
        overtime_wage = Decimal(r.ot_wage or 0)
        ...
        output.append(
            s.MonthlyReportOut(
                ...
                total_wage=(hourly_rate * total_hours) + overtime_wage,
            )
        )
    ...
    try:
        ...
        for attendance, labour in rows:
            ...
            if attendance.status == AttendanceStatus.ABSENT:
                ...
                working_hours = Decimal("4")
                ot_hours = Decimal(str(attendance.overtime_hours or 0))
                ot_rate = Decimal(str(attendance.overtime_rate or 0))
                wage = hourly_rate * working_hours + (ot_rate * ot_hours)

            else:
                working_hours = Decimal(str(attendance.working_hours or 0))
                ot_hours = Decimal(str(attendance.overtime_hours or 0))
                ot_rate = Decimal(str(attendance.overtime_rate or 0))
                wage = hourly_rate * working_hours + (ot_rate * ot_hours)

            key = (labour.id, payload.project_id)

            payroll_map.setdefault(
                key,
                {
                    "working_hours": Decimal("0"),
                    "overtime_hours": Decimal("0"),
            ...
            payroll_map[key]["overtime_hours"] += Decimal(
        ...
        for (labour_id, project_id), data in payroll_map.items():
            ...
            remaining_amount = max(Decimal("0"), total_wage - advance)

            payroll = LabourPayroll(
                labour_id=labour_id,
                project_id=project_id,
                month=payload.month,
                year=payload.year,
...
async def lock_payroll(
    ...
    if len(payload.payroll_ids) != len(set(payload.payroll_ids)):
    ...
    try:
        ...
        payrolls = await db.scalars(
            select(LabourPayroll)
            .join(Project, Project.id == LabourPayroll.project_id)
            .where(
                LabourPayroll.id.in_(payload.payroll_ids),
                Project.company_id == current_user.company_id,
            )
            .order_by(LabourPayroll.id)
            .with_for_update()
        )
        payrolls = payrolls.all()

        if len(payrolls) != len(payload.payroll_ids):
            raise HTTPException(
                status_code=400, detail="One or more payroll records not found"
            )
        ...
        for p in payrolls:
            if p.status != PayrollStatus.DRAFT:
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot lock Payroll {p.id}. Current Status: {p.status.value}. Only DRAFT payrolls can be locked.",
                )
        ...
        await db.rollback()
        logger.exception("Payroll lock failed")
        raise
...
async def unlock_payroll(
    ...
    try:
        ...
        for p in payrolls:
            if p.status != PayrollStatus.LOCKED or p.paid_amount > 0:
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot unlock Payroll {p.id}. Current Status: {p.status.value}. Paid Amount: {p.paid_amount}. Only LOCKED payrolls with zero paid_amount can be unlocked.",
                )
        ...
        await db.rollback()
        logger.exception("Payroll unlock failed")
        raise
...
async def pay_salary(
    ...
    if payroll.status == PayrollStatus.DRAFT:
        raise HTTPException(
            status_code=400, detail="Cannot pay DRAFT payroll. Please lock it first."
        )

    if payroll.status == PayrollStatus.PAID:
        raise HTTPException(status_code=400, detail="Payroll is already PAID in full.")
    ...
    db.add(
        JournalLine(
            entry_id=entry.id,
            account_id=wages_payable_acc.id,
...
async def advance_payment(
    ...
    # Check whether this labour's user is assigned to the project
...
@router.get(
    ...
    response_model=s.AttendanceDashboardOut,
)
async def attendance_dashboard(
    ...
    if labour_id is not None:
        ...
        attendance_filters.append(UserAttendance.user_id == labour.user_id)
    ...
    if labour_id is None:
        total_labour = (
            await db.scalar(
                ...
                .join(
                    Labour,
                    Labour.user_id == ProjectMember.user_id,
                )
    ...
    present = (
        await db.scalar(
            select(func.count(func.distinct(UserAttendance.user_id))).where(
                *attendance_filters,
    ...
    total_working_hours = (
        await db.scalar(
            select(func.sum(UserAttendance.working_hours)).where(*attendance_filters)
        )
    ) or Decimal("0")
    ...
    total_overtime_hours = (
        await db.scalar(
            select(func.sum(UserAttendance.overtime_hours)).where(*attendance_filters)
        )
    ) or Decimal("0")

    # =====================================================
    # CONTRACT / INDEPENDENT LABOUR
    # =====================================================
    ...
    if labour_id is None:
        ...
        contract_labour = (
            await db.scalar(
                ...
                .where(
                    ...
                    Labour.contractor_id.is_not(None),
                )
            )
        ) or 0
        ...
        contract_labour = 1 if labour.contractor_id else 0
        independent_labour = 0 if labour.contractor_id else 1
    ...
    attendance_percentage = (
        round((present / total_labour) * 100, 2) if total_labour else 0
    )

    return s.AttendanceDashboardOut(
        ...
        independent_labour=independent_labour,
    )
...
async def export_payroll_excel(
    ...
    query = select(LabourPayroll, Labour).join(
        Labour, Labour.id == LabourPayroll.labour_id
    )
    ...
    if start_date:
        ...
        query = query.where(
            (LabourPayroll.year * 12 + LabourPayroll.month) >= start_idx
        )
    if end_date:
    ...
    result = await db.execute(
        query.order_by(LabourPayroll.year.desc(), LabourPayroll.month.desc())
    )
    ...
    try:
        async with db.begin_nested():
            ...
            if overlapping:
                raise HTTPException(
                    status_code=409,
                    detail="Wage period overlaps with an existing wage record",
                )
            ...
            for att in attendance_rows:
                if att.status == AttendanceStatus.ABSENT:
                    ...
                    total_wage += (hourly_rate * Decimal("4")) + (
                        Decimal(str(att.overtime_rate or 0))
                        * Decimal(str(att.overtime_hours or 0))
                    )
                else:
                    total_wage += (
                        hourly_rate * Decimal(str(att.working_hours or 0))
                    ) + (
                        Decimal(str(att.overtime_rate or 0))
                        * Decimal(str(att.overtime_hours or 0))
                    )
        ...
        if (
            ...
            and e.orig.args[0] == 1213
        ):
            logger.warning(
                f"Deadlock detected during wage generation for labour {payload.labour_id}"
            )
            raise HTTPException(
                status_code=409,
...
async def list_wages(
    ...
    query = (
        ...
        .join(Labour, Labour.id == LabourWageRecord.labour_id)
        .join(Project, Project.id == LabourWageRecord.project_id)
        .options(selectinload(Labour.labour_type))
        .where(Project.company_id == current_user.company_id)
    )
    count_query = (
        select(func.count(LabourWageRecord.id))
        .join(Labour, Labour.id == LabourWageRecord.labour_id)
        .join(Project, Project.id == LabourWageRecord.project_id)
        .where(Project.company_id == current_user.company_id)
    )
    ...
    if start_date and end_date:
        query = query.where(
            and_(
                LabourWageRecord.start_date <= end_date,
                LabourWageRecord.end_date >= start_date,
            )
        )
...
async def pay_wage_record(
    ...
    db.add(
        JournalLine(
            entry_id=entry.id, account_id=wages_payable_acc.id, debit=amount, credit=0
        )
    )
    db.add(
        JournalLine(
            entry_id=entry.id, account_id=credit_account_id, debit=0, credit=amount
        )
    )
...
async def get_wage_stats(
    ...
    pending = await db.scalar(
        select(func.sum(LabourWageRecord.net_wage)).where(
            LabourWageRecord.project_id == project_id,
            LabourWageRecord.status == "PENDING",
        )
    ) or Decimal("0")

    paid = await db.scalar(
        select(func.sum(LabourWageRecord.net_wage)).where(
            LabourWageRecord.project_id == project_id, LabourWageRecord.status == "PAID"
        )
    ) or Decimal("0")

    advance = await db.scalar(
        select(func.sum(Expense.amount)).where(
            Expense.project_id == project_id, Expense.category == "Labour Advance"
        )
    ) or Decimal("0")

    contractor = await db.scalar(
        select(func.sum(Expense.amount)).where(
            Expense.project_id == project_id, Expense.category == "Contractor"
        )
    ) or Decimal("0")
...
@router.get(
    "/payroll/aggregate-report",
    response_model=list[s.AggregateReportOut],
)
async def get_aggregate_report(
    ...
    query = (
        select(*columns)
        .join(ProjectMember, ProjectMember.user_id == Labour.user_id)
        .join(LabourType, Labour.labour_type_id == LabourType.id)
        .outerjoin(
            UserAttendance,
            and_(
                UserAttendance.user_id == Labour.user_id,
                UserAttendance.project_id == project_id,
                extract("month", UserAttendance.attendance_date) == month,
                extract("year", UserAttendance.attendance_date) == year,
            ),
    ...
    if period_expr is not None:
        query = query.order_by(period_expr, Labour.labour_name)
    else:
    ...
    return [
        s.AggregateReportOut(
            ...
            ot_hours=row.ot_hours or Decimal("0"),
            total_wage_earned=row.total_wage_earned or Decimal("0"),
            status=(
